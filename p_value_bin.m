% p_value_bin: a function taking as inputs 
% (a)the three time series X,Y,Z (row vectors) that each of their elements take values 0 or 1 (BINARY time series)
% (b) the alphabet sizes of series X, Y, Z (typically 2)
...
function p_val= p_value_bin(X_alph_size, Y_alph_size, Z_alph_size, X, Y, Z, D )
...
%(d) Each element of "cell_non_zeros" lists the indices of ones in the context in the respective row of "a_context"



for i=(D+1):length(X)
    
    % Find the ones in the past (D+1) values of X, Y and Z (call this
    % vector "past")
    non_zero=find([X((i-D):i), Y((i-D):i), Z((i-D):i)]);
    ...
    if isempty(non_zero)==1
        ...
        % This for loop identifies if the current context has already appeared in the data
        ...
        % and increase "row_num" by 1
            ...
            a_context(row_num+1, non_zero)= ones(1,length(non_zero));           
            count(row_num+1)=1;
            row_num=row_num+1;
